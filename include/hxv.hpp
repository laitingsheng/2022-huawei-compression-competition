...
template<typename T>
[[using gnu : always_inline, hot]]
inline static void from_vector(const T * data, uint32_t count, char * write_pos)
{
...
)
{
	...
	if (!read_pos)
		return 1;
	from_vector(buffer, line_count, output + 5);

	read_pos = utils::lz4::decompress_vector(read_pos, buffer, line_count);
	if (!read_pos)
		return 1;
	from_vector(buffer, line_count, output + 10);

	read_pos = utils::lz4::decompress_vector(read_pos, buffer, line_count);
	if (!read_pos)
		return 1;
	from_vector(buffer, line_count, output + 15);

	read_pos = utils::lz4::decompress_vector(read_pos, buffer, line_count);
	if (!read_pos)
		return 1;
	from_vector(buffer, line_count, output + 20);
