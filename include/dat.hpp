...
)
{
	...
	utils::counter::entropy<int64_t, int64_t, uint32_t> counter0;
...
[[nodiscard]]
[[using gnu : always_inline, hot]]
inline static const char * from_entropy_counter(const char * read_pos, std::vector<int64_t> & output)
{
	auto size = *reinterpret_cast<const uint32_t *>(read_pos);
	read_pos += 4;
...
)
{
	...
	auto read_pos = from_entropy_counter(begin, columns[0]);
	if (!read_pos)
		return 1;
